...
func (c *bddCtx) saveDefinitionFromFile(name, relPath string) error {
	...
	return c.store.Save(scriptstore.Definition{
		Type:     "script",
		Language: "lua",
		Name:     name,
		Source:   string(data),
...
func (c *bddCtx) iStartScript(name, query string) error {
	queryRef, err := c.ensureBDDQueryRef(query)
	if err != nil {
		c.lastErr = err
		return nil
	}
	hash, err := c.engine.StartScript(name, queryRef)
	c.lastHash = hash
	c.lastErr = err
	return nil
}
...
func (c *bddCtx) iStopScript(name, query string) error {
	...
	c.lastErr = c.engine.StopScript(name, queryRef)
	return nil
}
...
func (c *bddCtx) instanceExists(name, query string) error {
	hash := scriptstore.HashInstance(name, bddQueryRef(query))
	entries, err := c.store.Search("sb-script.instances.*")
	if err != nil {
		return fmt.Errorf("search instances: %w", err)
	}
	for _, entry := range entries {
		var inst scriptstore.Instance
		if err := json.Unmarshal(entry.Data, &inst); err == nil && inst.Hash == hash {
			return nil
		}
	}
	return fmt.Errorf("expected instance %s/%s (%s) to exist", name, query, hash)
}
...
func (c *bddCtx) ensureBDDQueryRef(raw string) (string, error) {
	if raw == "" {
		return "", nil
	}
	ref := bddQueryRef(raw)
	if err := storage.EnsureQueryLayout(c.store); err != nil {
		return "", err
	}
	if err := storage.SaveQueryDefinition(c.store, ref, storage.Query{Pattern: raw}); err != nil {
		return "", err
	}
	return ref, nil
}

func bddQueryRef(raw string) string {
	if raw == "" {
		return ""
	}
	return "bdd_query_" + scriptstore.HashInstance("bdd", raw)
}
